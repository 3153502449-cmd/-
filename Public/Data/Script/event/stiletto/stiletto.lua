...
function x311200_OnStiletto( sceneId, selfId,ntype, idBagPos, idBagPosStuff )
	
	local EquipPoint = LuaFnGetBagEquipType( sceneId, selfId, idBagPos )
    if EquipPoint==2 then
		x311200_NotifyTip( sceneId, selfId, "时装不可打孔")
		return
	end
	
	local ret = LuaFnIsItemLocked( sceneId, selfId, idBagPos )
	if ret ~= 0 then
	return
	end		
	...
	if ntype ==1 then 
		for i=0,10 do 
		local ret= AddBagItemSlot( sceneId, selfId, idBagPos )	
		 if ret ==-4 then 
			break
		 end
		end
	...
	elseif ntype ==2 then 
	local n_JB=1000000	
	local ret = LuaFnIsItemLocked( sceneId, selfId, idBagPosStuff )
	if ret ~= 0 then
		x311200_NotifyTip( sceneId, selfId, "打孔材料已加锁" )
	return
	end		
	local type =0
	local ItemId = LuaFnGetItemTableIndexByIndex( sceneId, selfId, idBagPosStuff )
		if ItemId == 20310111 or ItemId == 20310115 then 
		n_JB=1000000
		type=1
		end 	
		...
		if HumanMoney  < n_JB  then
			x311200_NotifyTip( sceneId, selfId, "金钱不足"..n_JB.."金" )
			return
		end	
	...
	if LuaFnGetAvailableItemCount(sceneId, selfId, ItemId) <1 then
		x311200_NotifyTip( sceneId, selfId, "材料不足一个或枷锁了" )
	return
	end	
	
	if LuaFnDelAvailableItem(sceneId,selfId,ItemId, 1 )~=1 then
		x311200_NotifyTip( sceneId, selfId, "材料扣除失败" )
	return
	end
	AddMoney( sceneId, selfId, 0-n_JB );	
		for i =0 , 5 do
		local ret= AddBagItemSlotFour( sceneId, selfId, idBagPos )
		 if ret ==-4 then 
			break
		 end
		end
