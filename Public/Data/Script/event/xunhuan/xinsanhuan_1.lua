...
x050220_g_CopySceneMap = "xuanlei.nav"
...
function x050220_CheckAccept( sceneId, selfId, targetId )
	...
	DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_1_DAYTIME )
	...
	if nowDate ~= oldDate then
		takenTimes = 0
		SetMissionData( sceneId, selfId, MD_JIZHI_1_DAYTIME, nowDate )
	end
	...
	-- 检测玩家是否符合刚放弃任务
	local iTime = GetMissionData( sceneId, selfId, MD_JIZHI_1_LAST )			-- 上一次放弃任务的时间(单位：一刻钟)
...
function x050220_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050220_g_TakeTimes 次
	local DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_1_DAYTIME )
	local takenTimes = floor( DayTimes/100000 )
	DayTimes = (takenTimes+1)*100000 + GetDayTime()
	SetMissionData( sceneId, selfId, MD_JIZHI_1_DAYTIME, DayTimes )
...
function x050220_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050220_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		for	i = 0, nearMemberCount - 1 do
			...
			if GetLevel(sceneId, memberID) >= 10 then
...
function x050220_OnAbandon( sceneId, selfId )
	...
	--end modified by zhangguoxin 090208
	SetMissionData( sceneId, selfId, MD_JIZHI_1_LAST, CurTime )
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	if leaveFlag == 1 then															-- 需要离开
		...
		if leaveTickCount >= x050220_g_CloseTick then										-- 倒计时间到，大家都出去吧
			...
			local strText = format( "你将在 %d 秒後离开场景", ( x050220_g_CloseTick - leaveTickCount ) * x050220_g_TickTime )
...
--				LuaFnSetCopySceneData_Param( sceneId, 11, TickCount )
		...
		elseif TickCount >= 10 and monsternum >= 6 and iskillNiuQu == 1 and iskillNiuQi == 1 and isBossWang == 0 then
