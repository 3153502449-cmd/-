...
x050222_g_CopySceneMap = "rongyan.nav"
...
function x050222_CheckAccept( sceneId, selfId, targetId )
	...
	DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_3_DAYTIME )
	...
	if nowDate ~= oldDate then
		takenTimes = 0
		SetMissionData( sceneId, selfId, MD_JIZHI_3_DAYTIME, nowDate )
	end
	...
	-- 检测玩家是否符合刚放弃任务
	local iTime = GetMissionData( sceneId, selfId, MD_JIZHI_3_LAST )			-- 上一次放弃任务的时间(单位：一刻钟)
...
function x050222_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050222_g_TakeTimes 次
	local DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_3_DAYTIME )
	local takenTimes = floor( DayTimes/100000 )
	DayTimes = (takenTimes+1)*100000 + GetDayTime()
	SetMissionData( sceneId, selfId, MD_JIZHI_3_DAYTIME, DayTimes )
...
function x050222_OnDie( sceneId, objId, selfId )--这个selfId可能是宠物
	...
	if szName == x050222_g_BossHuoYanYao then
		...
		for i = 0, num - 1 do
		    --处理吧。
			local Salary = GetMissionData(sceneId,mems[i],MD_NewSalaryTask)
			local SalarySeparate = mod(floor(Salary/100000),10)
			if SalarySeparate < 2 then
			  SetMissionData(sceneId,mems[i],MD_NewSalaryTask,Salary+1*100000)
			end
			--
...
function x050222_OnAbandon( sceneId, selfId )
	...
	if LuaFnDelAvailableItem( sceneId, selfId, x050222_g_xuanfouzhu, 1 ) > 0 then
		...
		--end modified by zhangguoxin 090208
		SetMissionData( sceneId, selfId, MD_JIZHI_3_LAST, CurTime )
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	if leaveFlag == 1 then															-- 需要离开
		...
		if leaveTickCount >= x050222_g_CloseTick then										-- 倒计时间到，大家都出去吧
			...
			local strText = format( "你将在 %d 秒後离开场景", ( x050222_g_CloseTick - leaveTickCount ) * x050222_g_TickTime )
