...
x050221_g_CopySceneMap = "duzhang.nav"
...
function x050221_CheckAccept( sceneId, selfId, targetId )
	...
	DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_2_DAYTIME )
	...
	if nowDate ~= oldDate then
		takenTimes = 0
		SetMissionData( sceneId, selfId, MD_JIZHI_2_DAYTIME, nowDate )
	end
	...
	-- 检测玩家是否符合刚放弃任务
	local iTime = GetMissionData( sceneId, selfId, MD_JIZHI_2_LAST )			-- 上一次放弃任务的时间(单位：一刻钟)
...
function x050221_OnAccept( sceneId, selfId, targetId )
	...
	-- 一天只能领 x050221_g_TakeTimes 次
	local DayTimes = GetMissionData( sceneId, selfId, MD_JIZHI_2_DAYTIME )
	local takenTimes = floor( DayTimes/100000 )
	DayTimes = (takenTimes+1)*100000 + GetDayTime()
	SetMissionData( sceneId, selfId, MD_JIZHI_2_DAYTIME, DayTimes )
...
function x050221_OnAbandon( sceneId, selfId )
	...
	--begin modified by zhangguoxin 090208
	SetMissionData( sceneId, selfId, MD_JIZHI_2_LAST, CurTime )
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	if leaveFlag == 1 then															-- 需要离开
		...
		if leaveTickCount >= x050221_g_CloseTick then										-- 倒计时间到，大家都出去吧
			...
			local strText = format( "你将在 %d 秒後离开场景", ( x050221_g_CloseTick - leaveTickCount ) * x050221_g_TickTime )
...
--				LuaFnSetCopySceneData_Param( sceneId, 16, TickCount )
		...
		--大于等于15分钟
		elseif TickCount >= 10 and isFireBoss == 1 and isYaoWang == 0 then
