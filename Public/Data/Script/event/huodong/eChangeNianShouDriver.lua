...
function x889051_ChangeLowNianShou(sceneId, selfId, targetId)
	...
	--身上是否有物品
	if (nCount<=0) then
		x889051_NotifyFailBox( sceneId, selfId, targetId, "年兽拼图不足")
		return
	end
	...
	if LuaFnGetPropertyBagSpace( sceneId, selfId ) < 1 then
		x889051_NotifyFailBox( sceneId, selfId,targetId, "背包空间不足" )
		return
	end
...
function x889051_ChangeHighNianShou(sceneId, selfId, targetId, targetId)
	...
	--身上是否有物品
	if (nCount<=0) then
		x889051_NotifyFailBox( sceneId, selfId, targetId, "物品不足")
		 return
	end
	...
	if (nCount<x889051_g_HighNianShouCount) then
		...
		--检测背包是否有地方....
	if LuaFnGetPropertyBagSpace( sceneId, selfId ) < 1 then
		x889051_NotifyFailBox( sceneId, selfId,targetId, "空间不足" )
		 return
	end
...
function x889051_CompositeFigure(sceneId, selfId, targetId)
	...
	if (nCount<=0) then
		...
		x889051_NotifyFailBox( sceneId, selfId, targetId, "年兽拼图碎片不足" )
		return
	end
...
function x889051_CompositeFigureDescript(sceneId, selfId, targetId)
	local str = "组合年兽拼图，兑换拼图需要的碎片数量300个。"
	BeginEvent(sceneId)
		AddText(sceneId,str)
...
function x889051_OnDefaultEvent( sceneId, selfId, targetId )
	...
	elseif nNum == 21 then
		x889051_CloseWindow(sceneId, selfId, targetId)
	elseif nNum == 5 then
			local str = "兑换初级坐骑年兽踏风，需要碎片49个#r兑换中级坐骑年兽逐日需要拼图1个。#r兑换高级坐骑年兽雷霆需要拼图15个。#r合成一个拼图需要300个碎片。"
			x889051_NotifyFailBox( sceneId, selfId, targetId, str )
	end 
end
