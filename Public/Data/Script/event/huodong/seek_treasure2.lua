...
--脚本号
x808047_g_ScriptId = 808047;

--副本名称
x808047_g_CopySceneName = "楼兰寻宝[快速模式]"
x808047_g_activity_time = { 
	[1] = {startTime = 1930, endTime = 2200},
	[2] = {startTime = 1130, endTime = 1430},
};

x808047_g_impact_Id = 73;
x808047_g_TotalNeedKill = 200;
x808047_g_MonsterChangeTime = 90 * 1000; -- 怪物变身时间 90秒 (毫秒)

x808047_g_CopySceneType = FUBEN_SEEK_TREASURE	--副本类型，定义在ScriptGlobal.lua里面

x808047_g_LimitMembers = 1							--可以进副本的最小队伍人数
x808047_g_TickTime = 1									--回调脚本的时钟时间（单位：秒/次）
x808047_g_LimitTotalHoldTime = 720 			--副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败 72*5 =60分钟
x808047_g_StartTickCount = 7  					--战斗开始提醒（单位：次数）
x808047_g_CloseTick = 6									--副本关闭前倒计时（单位：次数）
x808047_g_NoUserTime = 36								--副本中没有人后可以继续保存的时间（单位：秒） 3分钟
x808047_g_Fuben_X = 65									--进入副本的位置X need modify
x808047_g_Fuben_Z = 94									--进入副本的位置Z
x808047_g_Back_X = 163									--源场景位置X
x808047_g_Back_Z = 77										--源场景位置Z
x808047_g_Back_SceneId = 186						--源场景Id 楼兰

x808047_g_FuBen_Data = 
{
	MapFile = "loulanxunbao.nav", AreaFile = "loulanxunbao_area.ini", MonsterFile = "loulanxunbao_monster.ini",	
}
	
x808047_g_MonsterFlushSpeed =
{
	{ from = 1, to = 10, speed = 3 },
	{ from = 11, to = 20, speed = 3 },
	{ from = 21, to = 30, speed = 2 },
	{ from = 31, to = 40, speed = 2 },
	{ from = 41, to = 50, speed = 2 },
}

x808047_g_MonsterFlushPos =
{
...
--x808047_g_NianShouCreatePos = --年兽刷出坐标
--{
--	{34,65},{38,49},{43,42},{73,35},{91,48},
--	{95,60},{92,78},{80,91},{48,91},{97,66},
--}
--x808047_g_NianShouId = --年兽ID
--{
--	12206, --70
--	12207,
--	12208,
...
-- 上古宝箱	
x808047_g_SmallMonsterId = 
{
}	
x808047_g_SmallMonsterId[1] = {12102,12103,12104,12105,12106,12107,12108,12109,12110} --70,80,90...150 银色
x808047_g_SmallMonsterId[2] = {12111,12112,12113,12114,12115,12116,12117,12118,12119} --70,80,90...150 金色
-- 宝箱童子
x808047_g_MiddleMonsterId =
{
}
x808047_g_MiddleMonsterId[1] = {12120,12121,12122,12123,12124,12125,12126,12127,12128}
x808047_g_MiddleMonsterId[2] = {12129,12130,12131,12132,12133,12134,12135,12136,12137}
-- 镇宝龙王
x808047_g_BossMonsterId = {12138,12139,12140,12141,12142,12143,12144,12145,12146}

--**********************************
--任务入口函数
--**********************************
function x808047_OnDefaultEvent( sceneId, selfId, targetId )
	local id = GetNumText();
	local msg = "";
	if id == 1 then
		-- 是否开放
		if x808047_IsOpenNow() == 0 then	  
			BeginEvent(sceneId)
		...
		-- 检测失败
		local ret, msg = x808047_CheckEnterCondition(sceneId, selfId);
		if ret == 0 then
			x808047_RetDlg(sceneId, selfId, targetId, msg);
			return
		end
		if x808047_CheckMemberInfo(sceneId, selfId, targetId) ~= 1 then
			return
		end
		-- 检测通过
		x808047_MakeCopyScene(sceneId, selfId)
	elseif id == 2 then
		BeginEvent(sceneId)
			msg = "#{LLXB_8815_08}";
			AddText(sceneId, msg);
...
--列举事件
--**********************************
function x808047_OnEnumerate( sceneId, selfId, targetId )	
	AddNumText(sceneId, x808047_g_ScriptId, "楼兰寻宝[快速模式]", 10, 1);
	AddNumText(sceneId, x808047_g_ScriptId, "楼兰寻宝活动帮助", 11, 2);
end

--**********************************
--怪物死亡....
--**********************************
function x808047_OnDie( sceneId, objId, killerId )
	--是否是副本
	...
	if fubentype~=x808047_g_CopySceneType then
		return
	end
	--副本关闭标志
	...
	if killednumber <= x808047_g_TotalNeedKill then
		for i=0,num-1 do
			...
			if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then
				BeginEvent(sceneId)
					strText = format("已杀死怪物 %d/%d", killednumber, x808047_g_TotalNeedKill )
		...
		----
		if killednumber == x808047_g_TotalNeedKill then --15秒后出龙王
			LuaFnSetCopySceneData_Param(sceneId, 15, 1); --出龙王标志置位
			--给定身buff 73
			for i=0, num-1 do
				humanObjId = LuaFnGetCopyScene_HumanObjId(sceneId,i)--取得当前场景里人的objId
				if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then 
					LuaFnSendSpecificImpactToUnit(sceneId, objId, objId, humanObjId, x808047_g_impact_Id, 0); 
				end
			end
		end
	elseif killednumber > x808047_g_TotalNeedKill then --龙王都被干掉了
		--设置任务完成标志
		...
		for i=0,num-1 do
			...
			if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then
				BeginEvent(sceneId)
					strText = format("任务完成，将在%d秒后传送到入口位置", x808047_g_CloseTick*x808047_g_TickTime )
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
			end
		...
		-- boss 击杀公告
		x808047_playNotify(sceneId, killerId);
	end
end
...
--Boss击杀公告
--**********************************
function x808047_playNotify(sceneId, killerId)
	-- zchw 全球公告
...
--有玩家在副本中死亡事件
--**********************************
function x808047_OnHumanDie( sceneId, selfId, killerId )
end

--**********************************
--副本场景定时器事件
--**********************************
function x808047_OnCopySceneTimer( sceneId, nowTime )
...
--			--PrintStr("ID="..x808047_g_NianShouId[iniLevel]..pos[1]..","..pos[2]..actLevel);
--			for i ,pos in x808047_g_NianShouCreatePos do
--				local objId = LuaFnCreateMonster( sceneId, x808047_g_NianShouId[iniLevel], pos[1], pos[2], 1, 272, -1 )
...
--	end
	...
	if leaveFlag == 1 then -- 需要离开		
		...
		if leaveTickCount >= x808047_g_CloseTick then 						--倒计时间到，大家都出去吧		
			oldsceneId = LuaFnGetCopySceneData_Param(sceneId, 3) ;	--取得副本入口场景号
			...
			for	i=0,membercount-1 do
				...
				if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
					NewWorld( sceneId, mems[i], oldsceneId, x808047_g_Back_X, x808047_g_Back_Z )
				end
			end
			
		elseif leaveTickCount < x808047_g_CloseTick then
			...
			for	i=0,membercount-1 do
				...
				if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
	  			BeginEvent(sceneId)
	  				strText = format("你将在%d秒后离开场景!", (x808047_g_CloseTick-leaveTickCount)*x808047_g_TickTime )
	  				AddText(sceneId,strText);
	  			EndEvent(sceneId)
	...
	elseif tickCount >=  x808047_g_LimitTotalHoldTime then --副本最大存活时间到
		...
		--设置副本关闭标志
		LuaFnSetCopySceneData_Param(sceneId, 12, 1) ;
	elseif tickCount < x808047_g_StartTickCount then 
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
		for i=0, membercount-1 do
			...
			if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
				local tm = (x808047_g_StartTickCount - tickCount)*x808047_g_TickTime;
				BeginEvent(sceneId)
	...
	elseif tickCount == x808047_g_StartTickCount then
		...
		local mstId = x808047_g_SmallMonsterId[1][grade];
		for i, pos in x808047_g_MonsterFlushPos[1] do
			local objId = LuaFnCreateMonster( sceneId, mstId, pos[1], pos[2], 7, -1, 808047 )
		...
		local bActive = x808047_IsOpenNow();
		...
		for i = 0, monsterCount - 1 do
			...
			if LuaFnIsCharacterLiving(sceneId, monsterObjId) > 0 then				--活的怪物
				...
				--------
				for j=1, 2 do
					for i, record in x808047_g_SmallMonsterId[j] do
						if monstertype == record then
							if nowTime >= mcreatetime + x808047_g_MonsterChangeTime then
								LuaFnDeleteMonster(sceneId, monsterObjId);
								local extAi = 106; --银宝箱童子扩展AI 106
								if j == 2 then
									extAi = 105;     --金宝箱童子扩展AI 105
								end									 
								monsterObjId = LuaFnCreateMonster(sceneId, x808047_g_MiddleMonsterId[j][i], PosX, PosZ, 0, extAi, 808047);
		...
		local ret = nil;
		--找到刷怪间隔时间
		for i, record in x808047_g_MonsterFlushSpeed do
		...
		-- 到时间，刷怪	
		if ret ~= nil then
			if oldFlushMonsterTime + x808047_g_MonsterFlushSpeed[ret]["speed"] <= tickCount then 
				...
				local mstId = x808047_g_SmallMonsterId[mstColor][grade];
				if monsterBatch <= 50 and monsterBatch >= 1 then
					for i, pos in x808047_g_MonsterFlushPos[monsterBatch] do
						local objId = LuaFnCreateMonster( sceneId, mstId, pos[1], pos[2], 7, -1, 808047 )
						SetLevel( sceneId, objId, mstLvl )
					end		
				...
				if monsterBatch == 31 then
					...
					for i=0, membercount-1 do
						local objId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
						if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
							x808047_ShowMsg(sceneId, objId, "#{LLXB_8815_14}");
						end
					end
				end
			end
		...
		if LuaFnGetCopySceneData_Param( sceneId, 14 ) == 1 then
			...
			if diffCount <= 6 and diffCount >= 1 then
				...
				for i=0, membercount-1 do
					local objId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
					if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
						local tm = diffCount*x808047_g_TickTime;
						msg = format("%d秒之后将会重新开始战斗！", tm);
						x808047_ShowMsg(sceneId, objId, msg);
					end
				end
			end
		end
		...
		if num > 0 then
			if num == 4 then -- 出龙王
				local grade = LuaFnGetCopySceneData_Param(sceneId, 10); 		-- 怪类型 
				local mstLvl = LuaFnGetCopySceneData_Param(sceneId, 13); -- 怪等级	
				local objId = LuaFnCreateMonster( sceneId, x808047_g_BossMonsterId[grade], x808047_g_BossPos.x, x808047_g_BossPos.z, 0, 271, 808047 )
				SetLevel( sceneId, objId, mstLvl )	
				--泡泡
...
--有玩家进入副本事件
--**********************************
function x808047_OnPlayerEnter( sceneId, selfId )
	local tmDay = GetTime2Day();
	SetMissionData(sceneId, selfId, MD_SEEK_TREASURE, tmDay); --参与时间
	--设置死亡后复活点位置
	SetPlayerDefaultReliveInfo( sceneId, selfId, "%10", -1, "0", sceneId, x808047_g_Fuben_X, x808047_g_Fuben_Z );

        if floor(GetMissionData(sceneId,selfId,GONGZI_2)/10^8) == mod(GetWeekTime(),10) then
           if floor(GetMissionData(sceneId,selfId,GONGZI_1)/10^8) == 3 and mod(GetMissionData(sceneId,selfId,GONGZI_1),100) < 2 then
              SetMissionData(sceneId,selfId,GONGZI_1,GetMissionData(sceneId,selfId,GONGZI_1)+1)
           end
        end
        CallScriptFunction( 890536,"JianCe",sceneId,selfId)
        if floor(mod(GetMissionData(sceneId,selfId,HUOYUEFB_2),10^7)/10^6) < 1 then
           SetMissionData(sceneId,selfId,HUOYUEZHI,GetMissionData(sceneId,selfId,HUOYUEZHI)+37) --活跃值+37
           SetMissionData(sceneId,selfId,HUOYUEFB_2,GetMissionData(sceneId,selfId,HUOYUEFB_2)+10^6)
        end
end
...
--副本事件
--**********************************
function x808047_OnCopySceneReady( sceneId, destsceneId )
	...
	if LuaFnHasTeam( sceneId, leaderObjId ) == 0  then   -- 没有队伍
		NewWorld( sceneId, leaderObjId, destsceneId, x808047_g_Fuben_X, x808047_g_Fuben_Z) ;
	else
		if IsCaptain(sceneId, leaderObjId) == 0  then
			...
			for	i=0,nearteammembercount-1 do
				...
				NewWorld( sceneId, mems[i], destsceneId, x808047_g_Fuben_X, x808047_g_Fuben_Z) ;
			end
		end		
	end
...
--创建副本
--**********************************
function x808047_MakeCopyScene(sceneId, selfId)
	-- 使用队员的等级来算出怪物的等级
	local param0 = 4;
	local param1 = 3;
	...
	LuaFnSetSceneLoad_Map(sceneId, x808047_g_FuBen_Data.MapFile); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	LuaFnSetCopySceneData_TeamLeader(sceneId, leaderguid);
	LuaFnSetCopySceneData_NoUserCloseTime(sceneId, x808047_g_NoUserTime*1000);
	LuaFnSetCopySceneData_Timer(sceneId, x808047_g_TickTime*1000);
	LuaFnSetCopySceneData_Param(sceneId, 0, x808047_g_CopySceneType);--设置副本数据，这里将0号索引的数据设置为999，用于表示副本号999(数字自定义)
	LuaFnSetCopySceneData_Param(sceneId, 1, x808047_g_ScriptId);--将1号数据设置为副本场景事件脚本号
	...
	--设置场景中的各种Npc和区域
	LuaFnSetSceneLoad_Area( sceneId, x808047_g_FuBen_Data.AreaFile )
	LuaFnSetSceneLoad_Monster( sceneId, x808047_g_FuBen_Data.MonsterFile )
...
--信息提示
--**********************************
function x808047_ShowMsg( sceneId, selfId, msg )
	BeginEvent(sceneId)
		AddText(sceneId, msg);
	EndEvent()
	DispatchMissionTips(sceneId, selfId);
...
--返回对话
--**********************************
function x808047_RetDlg(sceneId, selfId, targetId, msg)
	BeginEvent(sceneId)
		AddText(sceneId, msg);
	EndEvent()
	DispatchEventList(sceneId, selfId, targetId)
...
--活动时间验证 成功：返回1； 失败：0
--**********************************
function x808047_IsOpenNow()
	local hour = GetHour();
	local minute = GetMinute();
	local time = hour*100 + minute;
	if (time > x808047_g_activity_time[1]["startTime"] and time < x808047_g_activity_time[1]["endTime"]) or
     (time > x808047_g_activity_time[2]["startTime"] and time < x808047_g_activity_time[2]["endTime"]) then			
		return 1;
	end
	--return 0;
	return 1;
...
--成功：1 失败：0
--**********************************
function x808047_CheckEnterCondition(sceneId, selfId)
 	--组队？
 	...
 	if teamSize < x808047_g_LimitMembers then
...
--成功：1 失败：0
--**********************************
function x808047_CheckMemberInfo(sceneId, selfId, targetId)
  --检查每一个成员信息
  local bSucc = 1;
  local teamSize = GetTeamSize(sceneId, selfId);
  local msg = "";
  local x808047_member_info = {
  ...
  for i=0, teamSize-1 do
  	...
  	local bXinfaOK = x808047_CheckXinfaLevel(sceneId, objId, 40) --心法达到40级？
  	local joinDate = GetMissionData(sceneId, objId, MD_SEEK_TREASURE);

  	x808047_member_info[i+1]["name"] = GetName(sceneId, objId);
  	if GetTime2Day() == joinDate then
  		x808047_member_info[i+1]["taskCount"] = "#cff0000不满足";
  		bSucc = 0;  		
  	end
  	if level < 75 then
  		x808047_member_info[i+1]["levelReq"] = "#cff0000不满足";
  		bSucc = 0;
  	end
  	if bXinfaOK == 0 then
  		x808047_member_info[i+1]["xinfaReq"] = "#cff0000不满足";
  		bSucc = 0;
  	end
  end
  ---------------------
	if bSucc == 0 then
		...
		for i, mem in x808047_member_info do
			if i > teamSize then
				break
			end	
			msg = format("  #B队员%s：", mem["name"]);
			if x808047_member_info[i]["levelReq"] == "#cff0000不满足" then
				msg = msg.."#r  #cff0000任务等级75             不满足";
			else
				msg = msg.."#r  #G任务等级75             满足";
			end
			---------------
			if x808047_member_info[i]["xinfaReq"] == "#cff0000不满足" then
				msg = msg.."#r  #cff0000心法等级40             不满足" ;
			else
				msg = msg.."#r  #G心法等级40             满足";
			end
			---------------
			if x808047_member_info[i]["taskCount"] == "#cff0000不满足" then
...
--成功：1 失败：0
--**********************************
function x808047_CheckXinfaLevel(sceneId, selfId, level)
	local nMenpai = GetMenPai(sceneId, selfId)
	if nMenpai<0 or nMenpai == 9 or nMenpai>12   then
		return 0
	end
