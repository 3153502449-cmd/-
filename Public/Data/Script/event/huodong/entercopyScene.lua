...
function x808121_OnCopySceneTimer(sceneId, nowTime)
	...
	local strOutMsg = ""
	--第二批怪逻辑
	if (curTickCount >=140 and curTickCount <= 200) then
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
		local memId;
		
		if(curTickCount == 140) then
			strOutMsg = "千年奇兽：雏凤将于60秒后降临。"
		elseif(curTickCount == 170) then
			strOutMsg = "千年奇兽：雏凤将于30秒后降临。"
		elseif(curTickCount == 190) then
			strOutMsg = "千年奇兽：雏凤将于10秒后降临。"
		elseif(curTickCount == 195) then
			strOutMsg = "千年奇兽：雏凤将于5秒后降临。"
		elseif(curTickCount == 200) then
		...
		--第三批怪逻辑
	elseif (curTickCount >=240 and curTickCount <= 300) then
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
		local memId;
	...
	--第四批怪逻辑
	elseif (curTickCount >=340 and curTickCount <= 400) then
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
		local memId;
		
		if(curTickCount == 340) then
			strOutMsg = "千年奇兽：雏凤将于60秒后降临。"
		elseif(curTickCount == 370) then
			strOutMsg = "千年奇兽：雏凤将于30秒后降临。"
		elseif(curTickCount == 390) then
			strOutMsg = "千年奇兽：雏凤将于10秒后降临。"
		elseif(curTickCount == 395) then
			strOutMsg = "千年奇兽：雏凤将于5秒后降临。"
		elseif(curTickCount == 400) then
			strOutMsg = "千年奇兽：雏凤已经降临到神秘的玄武岛了。"
			--出现第四批怪
