...
XiaoXiangStr={"更名失败","DB压力过大，请重新尝试","不可接受的新名称","名称重复","恭喜您，您的名字修改成功！"}
--**********************************
--事件交互入口
--**********************************
function x808008_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x808008_g_Key["ChgU"] then
		BeginEvent( sceneId )
			AddText( sceneId, "#B您需要去商店购买一个更名贴,就可以帮您更改名字." )
			AddNumText( sceneId, x808008_g_ScriptId, "确定更改角色名", 6, x808008_g_Key["ChgU_Y"] )
		EndEvent( sceneId )
...
function x808008_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x808008_g_ScriptId, "更改角色名字", 6, x808008_g_Key["ChgU"] )
	AddNumText( sceneId, x808008_g_ScriptId, "更新师徒称号", 6, x808008_g_Key["TitP"] )
	AddNumText( sceneId, x808008_g_ScriptId, "更新夫妻称号", 6, x808008_g_Key["TitS"] )
...
function x808008_IsCanChangeUserName( sceneId, selfId )
  ...
  if LuaFnGetAvailableItemCount(sceneId, selfId, 30008105) >= 1 then
    ...
    x808008_MsgBox( sceneId, selfId, "您包裹里没有更名贴,无法完成更名！请去热卖商店购买." )	
	return 0
...
if x808008_XiaoXiangGaiMinFeiFaJC(sceneId,selfId)==1 then return 1 end return 0
end
...
function x808008_CallBackChangeUserNameAfter( sceneId, selfId, nRetType )
	...
	--};
for i=1,4 do if nRetType==i then x808008_MsgBox(sceneId,selfId,XiaoXiangStr[i]) x808008_XiaoXiangModifyNameFailure(sceneId,selfId,i) end end
if nRetType==0 then x808008_MsgBox(sceneId,selfId,XiaoXiangStr[5]) end return 1
end
function x808008_XiaoXiangGaiMinFeiFaJC(sceneId,selfId)
if LuaFnDelAvailableItem(sceneId,selfId,30008105,1)>=1 then return 1 else x808008_MsgBox(sceneId,selfId,"非法参数") return 0 end
end
function x808008_XiaoXiangModifyNameFailure(sceneId,selfId,xiaoxianggm)
if xiaoxianggm<5 and xiaoxianggm>0 then BeginAddItem(sceneId) AddItem(sceneId,30008105,1) EndAddItem(sceneId,selfId) AddItemListToHuman(sceneId,selfId) end
end
--**********************************
--醒目提示
--**********************************
