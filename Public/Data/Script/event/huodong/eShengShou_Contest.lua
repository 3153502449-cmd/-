...
--重楼材料
x808066_g_Chonglou = {10553100,38000012,39000035,39000009}
--技能书ID
...
function x808066_GiveSkillBook( sceneId, selfId, activatorId )
	...
	local randBook = 0
	local BookID = 0
	local ContVale = random(1,100) 
	if ContVale > 70 then
	 numBooks = getn(x808066_g_Chonglou)
	 randBook = random(numBooks)
	 BookID = x808066_g_Chonglou[ randBook ]	
	else
	...
	if BagIndex ~= -1 then
		...
		local str = format( "#W#{_INFOUSR%s}#cff99cc在#G圣兽山#cff99cc举烟放火不小心熏了眼睛，模糊之中撞在#Y大宝箱#cff99cc上，打开一看，竟然是一本#{_INFOMSG%s}。", PlayerName, ItemTransfer )
...
function x808066_GiveBOSS( sceneId, selfId, activatorId )
	...
	--公告....
	local PlayerName = GetName(sceneId, activatorId)
	local str = format( "#G圣兽山#cff99cc上群英夺宝，唯#W#{_INFOUSR%s}#cff99cc技高一筹，混战之中一掌将#Y白蟒宝箱#cff99cc打翻，不料竟跳出四个#W宝箱童子#cff99cc！", PlayerName )
	BroadMsgByChatPipe( sceneId, activatorId, str, 4 )
...
function x808066_GiveDropBox( sceneId, selfId, activatorId )
	...
	--公告....
	local PlayerName = GetName(sceneId, activatorId)
	local str = format( "#G圣兽山#cff99cc上#W#{_INFOUSR%s}#cff99cc在群豪推举之下打开#Y白蟒宝箱#cff99cc，只见金光闪过、遍地财宝、欢声一片。", PlayerName )
	BroadMsgByChatPipe( sceneId, activatorId, str, 4 )
