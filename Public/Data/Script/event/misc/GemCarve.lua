...
function x800117_OnGemCarve( sceneId, selfId, GemItemPos, NeedItemPos,TargetID )
	...
	--LuaFnEraseItem( sceneId, selfId, NeedItemPos )
        local myDZF_1 = LuaFnGetItemTableIndexByIndex(sceneId,selfId,NeedItemPos)
        if LuaFnDelAvailableItem(sceneId,selfId,myDZF_1,1) ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "雕琢符扣取不成功，宝石雕琢失败！" )
          return
        end
