...
function x800118_OnGemMelting( sceneId, selfId, GemItemPos1, NeedItemPos )
	if GemItemPos1 == -1 or  NeedItemPos == -1 then
		return
	end
	...
	local nSuiPianCount = LuaFnGetAvailableItemCount(sceneId, selfId, GemItemID1)
	...
	--检测是否是有同样的宝石....
	if nSuiPianCount <3  then
		x800118_NotifyTip( sceneId, selfId, "宝石数量不足3个，宝石熔炼失败。" )
		return
	end
	...
	--检测产物是否需要绑定....
	local bNeedBind = 0
	_,bNeedBind=ScriptGlobal_IsGemItemNumberTable( sceneId, selfId,GemItemPos1,3 )
    ...
    local dsad =  LuaFnDelAvailableItem(sceneId,selfId,GemItemID1,3)
	if dsad~= 1 then
		x800118_NotifyTip( sceneId, selfId, "扣除物品失败，无法熔炼宝石。" )
		return
	end
	
        local myRLF = LuaFnGetItemTableIndexByIndex(sceneId,selfId,NeedItemPos)
        if myRLF ~= 30900056 and myRLF ~= 30900057 then
            x800118_NotifyTip( sceneId, selfId, "你放入的物品不是熔炼符！" )
            return
        end
        if LuaFnDelAvailableItem(sceneId,selfId,myRLF,1) ~= 1 then
            x800118_NotifyTip( sceneId, selfId, "#Y熔炼符扣除失败，无法熔炼宝石！" )
            return
        end
	...
	--公告....
	if GetItemQuality(ProductID) >= 3 then
		local MeltingNPCTbl =
		{
			[0]   = "洛阳（178，185）彭怀玉",
			[420] = "束河古镇（134，84）荆嵌实",
