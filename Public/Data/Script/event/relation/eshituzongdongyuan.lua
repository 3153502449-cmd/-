...
x806020_g_Prize_List = 	{
						 	{id=20501007, count=1, rand=3000 , 	bind = 0},
						 	{id=20502007, count=1, rand=3000  , bind = 0},
						 	{id=38000952, count=1, rand=2980  , 	bind = 0},
							{id=30509014, count=1, rand=1000  , 	bind = 0},
						 	{id=20700001, count=1, rand=10 , 	bind = 0},
						}       
...
function x806020_OnDefaultEvent( sceneId, selfId, targetId )
	...
	elseif (GetNumText() == 3) then
		...
		if curDayTime == floor( dayCount / 100) then
			if dayCount - curDayTime * 100 >= 10 then
				BeginEvent(sceneId)
					AddText(sceneId, "对不起,您每天只能抽奖10次,请明天再来吧!")
		...
		for i=1, getn(prizeList) do
			if randIndex <= prizeList[i].rand then
				...
				if LuaFnGetMaterialBagSpace( sceneId, selfId ) < prizeList[i].count then
		            x806020_NotifyTips(sceneId, selfId, "#{STZDY_20080513_24}")
					return
				end
				--开始添加奖励
