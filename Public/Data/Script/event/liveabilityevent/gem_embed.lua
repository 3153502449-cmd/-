...
x701614_g_EquipGemTable[HEQUIP_CAP] = { 11, 12, 13, 14 }  --HEQUIP_DRESS
x701614_g_EquipGemTable[HEQUIP_DRESS] = { 11, 12, 13, 14 }
...
x701614_g_EquipGemTable[HEQUIP_ARMOR] = { 31,32,33,34 }
x701614_g_EquipGemTable[HEQUIP_DARK] = { 1, 2, 3, 4, 11, 12, 13, 14, 21 }
x701614_g_EquipGemTable[HEQUIP_WUHUN] = { 1, 2, 3, 4, 11, 12, 13, 14, 21 }
x701614_g_EquipGemTable[8] = { 1, 2, 3,13, 21,12 }
x701614_g_EquipGemTable[9] = { 1, 2,3, 13, 21,12 }
x701614_g_EquipGemTable[10] = {1, 2,3,13, 21,12 }
...
function x701614_GemEmbed( sceneId, selfId, GemBagIndex, EquipBagIndex, MatBagIndex1, MatBagIndex2 )
	...
	local BSpf= {4,7,10,15,20,25,30,35}
	...
	if(GemIsBind == 1) then
	  	if(itemTableIndex == 10422017 or itemTableIndex == 10423025) then
	...
	local insx = GetItemQuality( gemIdx)
	if insx >7 then
	x701614_MsgBox( sceneId, selfId, "大于7级的会被干掉...因为只开放7级" )	
 ...
 --  72 --  检查  
    local  MatBagIndev=0
	if LuaFnGetAvailableItemCount(sceneId, selfId, 30900010) >0 then 
	MatBagIndev=1	
	end
if MatBagIndex1 == 72 then 

	if MatBagIndev ==0 then 
		x701614_MsgBox( sceneId, selfId, "#{BSLCYH_130529_121}" )
		return 0
	end
	
elseif MatBagIndex1 == 73 then 

	if MatBagIndev ==0 then 
		if YuanBao(sceneId,selfId,-1,3,0) <98 then 
			x701614_MsgBox( sceneId, selfId, "元宝不足无法购买!" )
			return 0
		end
		if YuanBao(sceneId,selfId,-1,2,98) ~= -1 then 
		pos=  TryRecieveItem( sceneId, selfId, 30900010, 1 )	
		if pos ~= -1 then 
		MatBagIndev=1
		end
		end
...
if MatBagIndev ==1 then 
	...
	if MatBagIndev > 0 then
	LuaFnDelAvailableItem(sceneId,selfId,30900010, 1)
	end
	...
	if odds >= randRet then
		...
		if ret and ret == 0 then
		 ...
		 --  LuaFnSetItemCreator( sceneId, selfId,EquipBagIndex, data2)
	       --LuaFnRefreshItemInfo( sceneId, selfId, EquipBagIndex )
