...
function x888899_CDKEY(sceneId,selfId,card)
        --读CDK文件
		local nHand = openfile("./ZY_DataBase/CDK/588-CDK.txt", "r")
		local CDKstring = {}
		local isCDKright = -1
        if nHand and nHand ~= nil then
		   for i = 1,100 do
		      local LineData = read(nHand, "*l")
		      if LineData == nil then
			     return
			  end
			  CDKstring[i] = LineData
			  if LineData == card then
				 isCDKright = i
			  end			  
		   end
		   if isCDKright == -1 then
		      x888899_NotifyFailBox( sceneId, selfId, -1, "请输入正确的CD-KEY。或已使用！" )
		      return
		   end
		   closefile(nHand)
		   x888899_NotifyFailBox( sceneId, selfId, -1, "恭喜您，CD-KEY成功激活，成功领取奖励，请查收。" )
		   SetMissionData(sceneId,selfId,MD_NewUserCard3,GetMissionData(sceneId,selfId,MD_NewUserCard3)+10)
		   AddGlobalCountNews(sceneId,GetName(sceneId,selfId).."#W，通过宣传推广，成功激活#G588财富卡#W。获得10点充值！")
		else
		   x888899_NotifyFailBox( sceneId, selfId, -1, "ERRO,未知错误请联系管理-CD-KEY" )
		   closefile(nHand)
		   return
		end
		local nHandle = openfile("./ZY_DataBase/CDK/588-CDK.txt", "w")
		local newData = ""
		CDKstring[isCDKright] = "已激活:"..CDKstring[isCDKright]
		for i = 1,100 do
		   newData = newData..CDKstring[i].."\n"
		end
		write(nHandle, newData)
		closefile(nHandle)
end
--**********************************
-- 新手卡或体育抽奖卡检查
...
function x888899_OpenCard(sceneId,selfId,card)
    if strfind(card,"CDK") ~= nil then
	   x888899_CDKEY(sceneId,selfId,card)
	   return
	end
...
function x888899_EvenrDayPrize(sceneId,selfId,nYuanBao)
		...
		SetMissionData(sceneId,selfId,MD_EVERDATPAY,MyDatPrize+nYuanBao) ---这里再给个判断吧，防止玩家12点没有刷新时间数据。
		 local Button = GetMissionData(sceneId,selfId,MD_EVERDATPAYBUTTN)
		 local MyDatPrize = MyDatPrize+nYuanBao
		 local ButtonReq = {}
		 local PayReward_Prize = {[1] = 10,[2] = 30,[3] = 50,[4] = 100,}
		 local value = {1,10,100,1000}
		 for i = 1,4 do
		     ButtonReq[5-i] = mod(floor(Button/value[i]),10)
		 end
         for i = 1, 4 do
		    if MyDatPrize >= PayReward_Prize[i] then
				if  ButtonReq[i] == 0 then
		       ButtonReq[i] = 1
			   end
			end
		 end
...
--	另外，这里没有 targetId，不知道写 -1 会不会有问题
	...
	if( 2 == ntype ) then
	  if LuaFnGetGlobalDataString(sceneId,selfId,"BuyYuanbaoType") == 2 then
	      BuyYuanBaoCount(sceneId,selfId,targetId,1,nYuanBao)
		 local strText = ""
		 if nYuanBao == 88 then
		    strText = "您成功购买了周卡。"
		 elseif nYuanBao == 188 then
		    strText = "您成功购买了月卡。"
		 else
		    x888899_NotifyFailBox( sceneId, selfId, selfId, "    处理故障，请稍候再试，如无法成功操作请联系客服人员进行处理。" )
			return
		 end
		 BeginEvent(sceneId)
         ...
         x888899_LogForBuyWeekAndMonth(sceneId, selfId,nYuanBao)		 
	  else
	    if nYuanBao == 0 and GetMissionData(sceneId,selfId,MD_NewUserCard3) > 0 then
		   nYuanBao = LuaFnGetGlobalDataString(sceneId,selfId,"BuyYuanbaoCard")
		   SetMissionData(sceneId,selfId,MD_NewUserCard3,GetMissionData(sceneId,selfId,MD_NewUserCard3) - nYuanBao)
		end
		if nYuanBao == 0 then
		   return
		end
		YuanBao(sceneId,selfId,targetId,1,nYuanBao*1000)
		BuyYuanBaoCount(sceneId,selfId,targetId,1,nYuanBao)
		x888899_NotifyLeftPoint(sceneId,selfId,nLeftPoint)
		x888899_EvenrDayPrize(sceneId,selfId,nYuanBao)
		--给客户端提示信息
		local strText = ""
		BeginEvent(sceneId)
			strText = "您成功的兑换了"..tostring(nYuanBao*1000).."点元宝。"
			AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		SetMissionData(sceneId,selfId,MD_CUMULATIVERECHARGE,GetMissionData(sceneId,selfId,MD_CUMULATIVERECHARGE)+nYuanBao)
		AddGlobalCountNews(sceneId,GetName(sceneId,selfId).."#W通过在线充值，赞助了本服"..nYuanBao.."元，全体工作人员感谢该玩家赞助。祝您生活愉快，游戏舒心！")
		LuaFnMsg2Player( sceneId, selfId,strText,MSG2PLAYER_PARA)
		x888899_LogForDuiHuanYuanBao(sceneId, selfId,nYuanBao)
		--200+元宝给3级红宝石-绑定
		
		if nYuanBao and nYuanBao >= 10 then
			...
			if countPrize and countPrize == 0 then
				...
				TryRecieveItem( sceneId, selfId, 30008067, QUALITY_MUST_BE_CHANGE )				
				if BagIndex == -1 then
					return
				end				
				...
				BeginEvent(sceneId);
				AddText(sceneId, "恭喜您获赠3级宝石一颗和玄灵丹，请查收。");
				EndEvent(sceneId);
				DispatchMissionTips(sceneId, selfId);
			end
		end
...
-- 元宝兑换记录
--**********************************
function x888899_LogForDuiHuanYuanBao(sceneId, selfId,Point)
	...
	local handle = openfile("./ZY_DataBase/YuanBaoSystem/YuanBaoExchange.txt", "a+")
	if nil ~= handle then
		write(handle,format("Player:%s GUID:%s GUILD:%s Point:%s Time:%s",nName,nGuid,nGUILDName,Point,tostring(nowYear * 10000 + nowMonth * 100 + nowDate).." "..nHour..":"..nMinute))
...
-- 周卡月卡记录
--**********************************
function x888899_LogForBuyWeekAndMonth(sceneId, selfId,Point)
	local nowYear = GetTodayYear();
	local nowMonth = GetTodayMonth() + 1;
	local nowDate = GetTodayDate();
	local nHour	 = GetHour()
	...
	if Point == 88 then
	   MyText = "周卡："
	elseif Point == 188 then
	   MyText = "月卡："
	end
	local nGUILDName = LuaFnGetGuildName(sceneId, selfId)
	local handle = openfile("./ZY_DataBase/YuanBaoSystem/WeedAndMonth.txt", "a+")
	if nil ~= handle then
		write(handle,format(MyText.." Player:%s GUID:%s GUILD:%s Point:%s Time:%s",nName,nGuid,nGUILDName,Point,tostring(nowYear * 10000 + nowMonth * 100 + nowDate).." "..nHour..":"..nMinute))
		write(handle,tostring("\n"))
		closefile(handle)
	end
end
...
function x888899_NotifyLeftPoint(sceneId,selfId,nLeftPoint)
   local UI_id = 2003
   if LuaFnGetGlobalDataString(sceneId,selfId,"AskPointType") == 3 then
      UI_id = 20181011
	  LuaFnSetGlobalDataString(sceneId,selfId,"MyNowPoint",(nLeftPoint/1000) - 1)
   else
      UI_id = 2003
   end
	...
	DispatchUICommand( sceneId, selfId, UI_id )
end
