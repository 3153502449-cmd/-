...
function x402030_OnDefaultEvent( sceneId, selfId, targetId )
	...
	local nCurTime = GetDayTime()
	if  nPreTime ~= nCurTime then
		nTimes = 0
		SetMissionData(sceneId, selfId, MD_JIAOFEI_TIMES, nTimes)
		SetMissionData(sceneId, selfId, MD_JIAOFEI_PRE_TIME, nCurTime)
	end
	if nTimes >= 1   then
		BeginEvent(sceneId)
			AddText(sceneId,"#B镜湖剿匪");
			AddText(sceneId,"  您今天已经参加过剿匪行动了，请明天再来参加。");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	for i=0, nPlayerNum-1  do
		...
		-- 看时间是不是过了一天，过了就先更新
		if nCurTime ~= nPreTime then
			nTimes = 0
			SetMissionData(sceneId, nPlayerId, MD_JIAOFEI_TIMES, nTimes)
			SetMissionData(sceneId, nPlayerId, MD_JIAOFEI_PRE_TIME, nCurTime)
		end
	...
	elseif GetName(sceneId, objId) == x402030_g_BossMonsterName  then
		...
		for i=0, nHumanCount-1  do
			...
			if GetMissionData(sceneId,nHumanId,MD_JIAOFEI_KILL_BOSS_TIME) == 6  then
				AwardTitle(sceneId, nHumanId, 13, 117)
				SetCurTitle(sceneId, nHumanId, 13, 117)
				DispatchAllTitle(sceneId, nHumanId)
				
			elseif GetMissionData(sceneId,nHumanId,MD_JIAOFEI_KILL_BOSS_TIME) == 18  then
				AwardTitle(sceneId, nHumanId, 13, 118)
				SetCurTitle(sceneId, nHumanId, 13, 118)
				DispatchAllTitle(sceneId, nHumanId)
				
			elseif GetMissionData(sceneId,nHumanId,MD_JIAOFEI_KILL_BOSS_TIME) == 50  then
				AwardTitle(sceneId, nHumanId, 13, 119)
				SetCurTitle(sceneId, nHumanId, 13, 119)
				DispatchAllTitle(sceneId, nHumanId)
				
			elseif GetMissionData(sceneId,nHumanId,MD_JIAOFEI_KILL_BOSS_TIME) == 100  then
				AwardTitle(sceneId, nHumanId, 13, 120)
				SetCurTitle(sceneId, nHumanId, 13, 120)
				DispatchAllTitle(sceneId, nHumanId)
			end
