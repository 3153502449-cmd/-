...
--				Margin = Margin * (0.625+0.02*(Level-30)/2)
				AddMoney(sceneId,selfId,4*(Margin+Corpus));
				---2金本钱。
...
--				DispatchMissionTips(sceneId,selfId)
				...
				--local msg = "恭喜您结束漕运，本轮漕运您获得花红#{_EXCHG"..Balance.."}，加上20%额外收益，一共为#{_EXCHG"..1.2*Balance.."}。扣除押金后，您获得花红#{_EXCHG"..1.2*Balance-Corpus.."}。"
 				local msg = "恭喜您结束漕运，本轮漕运您获得收益#{_EXCHG"..Balance.."}，经过扣除押金#{_EXCHG20000}和加入由于使用交子获得的额外收益#{_EXCHG"..0.2*Balance.."}后，您最终获得的花红是#{_EXCHG"..4*Balance-Corpus.."}。"
 				local cmsg = "贪财贪财......"
				x311010_Client_Show_Message( sceneId, selfId, targetId, msg, cmsg,0)

			elseif Balance > 0 then
				AddMoney(sceneId,selfId,Balance);
				--记录统计信息
