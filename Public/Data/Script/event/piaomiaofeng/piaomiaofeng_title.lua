...
--兑换需求
x402305_g_Cost = {1,15,45,90,300}
--称号id
x402305_g_TitleID = {233,234,235,236,255}
--不同等级的失败提示
x402305_g_FailMsg = {"#{PMF_REMINDINF_002}","#{PMF_REMINDINF_003}","#{PMF_REMINDINF_004}","#{PMF_REMINDINF_005}","#{PMF_20110121_2}"}
--称号等级
x402305_g_TitleLvNum = 5;
--称号idx 程序用
x402305_g_TitleIndex = 17;--18为城主称号占位 懒得去找地方了。
...
function x402305_ChangePiaoMiaoTitle(sceneId, selfId, targetId,titleLv)
	...
	if (titleLv == 5) then
		--是否已有称号
		if (HadTitleID == x402305_g_TitleID[5]) then
			x402305_NotifyFailBox(sceneId, selfId, targetId, "#{PMF_REMINDINF_001}")	
			return
		end
		--是否有上一等级的称号
	...
	--设置当前称号
	SetCurTitle(sceneId,selfId,x402305_g_TitleIndex,x402305_g_TitleID[titleLv])
	--关闭对话框
...
function x402305_SendNotice(sceneId, selfId, targetId,lv)
	...
	if (lv == 5) then
		strformat	= "#P程青霜：#{_INFOUSR%s}#P大侠霸气外露，#G缥缈之唯我独尊#P的称号当之无愧。今后只需大侠报出名号，众宵小皆做鸟兽散！"
	end
...
function x402305_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if (nNum == 10) then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x402305_g_scriptId, "#{PMF_20110121_3}", 6, 25 )
		EndEvent(sceneId)
	...
	if (nNum == 25) then
		BeginEvent(sceneId)
			AddText(sceneId,"#{PMF_20110121_5}")
			AddNumText( sceneId, x402305_g_scriptId, "我要兑换", 6, 5 )
			AddNumText( sceneId, x402305_g_scriptId, "还是算了", 0, 100 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId,x402305_g_scriptId,0)
	end
