...
function x890064_OnDie( sceneId, selfId, killerId )
	...
	if playerName ~= nil then
		str = format("#H#{_INFOUSR%s}带领队伍在少室山上轻松将#G鸠摩智#H击败。", playerName); --鸠摩智
...
function x890064_UseMySkill( sceneId, selfId, skill )
	...
	elseif skill == "B" then
		...
		MonsterTalk(sceneId, -1, "", "#cC4A97B[鸠摩智]#W啊！我的悬枢穴好痛啊！" )
	...
	elseif skill == "C" then
		...
		MonsterTalk(sceneId, -1, "", "#cC4A97B[鸠摩智]#W啊，我的膻中气海好痛啊！" )
		local x,z = GetWorldPos( sceneId, selfId )
	...
	elseif skill == "D" then
		...
		if enemyId > 0 then
			...
			MonsterTalk(sceneId, -1, "", "#cC4A97B[鸠摩智]#W啊！我的丝竹空穴好痛啊！" )
			local x,z = GetWorldPos( sceneId, selfId )
...
function x890064_SkillA_NaLiZou( sceneId, selfId )
	...
	MonsterTalk(sceneId, -1, "", "#cC4A97B[鸠摩智]#W哪里走！" )
