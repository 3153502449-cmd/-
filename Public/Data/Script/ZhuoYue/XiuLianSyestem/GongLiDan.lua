--功力丹
--2019-9-9 19:07:26 逍遥子
--脚本号
x390102_g_scriptId = 390102 --临时写这个,真正用的时候一定要改.
--**********************************
--事件交互入口
--**********************************
...
function x390102_OnConditionCheck( sceneId, selfId )
	...
	local nGongLi = GetMissionData( sceneId, selfId, XIULIAN_GONGLI )	
    if nGongLi >= 10000 then
		x390102_NotifyTips( sceneId, selfId, "#{XLGLZHF_20091021_01}" )--您的功力是满的，不需要使用功力丹！
        return 0
	else
	    return 1;
	end
...
function x390102_OnActivateOnce( sceneId, selfId )
	...
	if nPower_Fin > 10000 then
		nPower_Fin = 10000
	end
	SetMissionData(sceneId, selfId, XIULIAN_GONGLI, nPower_Fin);
	x390102_NotifyTips( sceneId, selfId, "#{XLGLZHF_20091021_03}" )--使用成功，功力值已经恢复到100/100
	return 1;
end
