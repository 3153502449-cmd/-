...
function x000155_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent( sceneId )
		...
		--AddNumText( sceneId, x000110_g_scriptId, "属性时装修复", 6, 4 )--这里是我后面改了时装，需要玩家置换下时装。作出修改。开服的话屏蔽掉
		--AddNumText( sceneId, x000110_g_scriptId, "属性时装传承属性", 6, 3 )
		--AddNumText( sceneId, x000110_g_scriptId, "属性坐骑进阶", 6, 6 )--这里是我后面改了时装，需要玩家置换下时装。作出修改。开服的话屏蔽掉
		--AddNumText( sceneId, x000110_g_scriptId, "属性坐骑传承属性", 6, 7 )
...
function x000155_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 3 then
		BeginUICommand( sceneId )			
			...
			UICommand_AddInt( sceneId, targetId )
		EndUICommand( sceneId )
		DispatchUICommand( sceneId, selfId,  20200608)
		return
	end
