...
-- 卓越天龙兑换重楼套
{n=9100,id=10553101},{n=9200,id=10553102},{n=9300,id=10553100},{n=9400,id=10553110},{n=9500,id=10553108},
}
...
{n=9,id=20700001,num=20,str="重楼之泪"},
{n=10,id=20700002,num=20,str="重楼之芒"},
{n=11,id=20700003,num=20,str="重楼之灵"},
{n=12,id=20700004,num=20,str="琉璃明珠"},
{n=13,id=20700005,num=20,str="重楼之魄"},
...
function x001067_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x001067_g_ScriptId, "魔兵天降", 6, 19000 )
...
function x001067_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if nNumText == 19000 then
		BeginEvent(sceneId)
			AddText(sceneId, "  #{JPZB_0610_02}")
			AddNumText(sceneId, x001067_g_ScriptId, "兑换重楼戒", 6, 9100)
			AddNumText(sceneId, x001067_g_ScriptId, "兑换重楼玉", 6, 9200)
			AddNumText(sceneId, x001067_g_ScriptId, "兑换重楼链", 6, 9300)
			AddNumText(sceneId, x001067_g_ScriptId, "兑换重楼甲", 6, 9400)
			AddNumText(sceneId, x001067_g_ScriptId, "兑换重楼肩", 6, 9500)
			AddNumText( sceneId, x001067_g_ScriptId, "离开……", 0, 0 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
	...
	if nNumText > 1000 and nNumText < 10000  then
		BeginEvent(sceneId)
			...
			if nNumText == 9100 then
				nLevel = 9
			end
			if nNumText == 9200 then
				nLevel = 10
			end
			if nNumText == 9300 then
				nLevel = 11
			end
			if nNumText == 9400 then
				nLevel = 12
			end
			if nNumText == 9500 then
				nLevel = 13
			end
			
			local szStr = "  要获得这些装备，你需要给我“" .. x001067_g_StoneList[nLevel].str.. "”“".. tostring(x001067_g_StoneList[nLevel].num) .. "”个  该物品可在#GBOSS或者副本#W中爆出。"
...
function x001067_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )
	...
	if x001067_g_EquipList[nItemIndex].n == 9200 then
		nLevel = 10
	end
	if x001067_g_EquipList[nItemIndex].n == 9300 then
		nLevel = 11
	end
	if x001067_g_EquipList[nItemIndex].n == 9400 then
		nLevel = 12
	end
	if x001067_g_EquipList[nItemIndex].n == 9500 then
		nLevel = 13
	end
	...
	-- 检查是不是有足够的石头可以扣除
	if LuaFnGetAvailableItemCount(sceneId, selfId, x001067_g_StoneList[nLevel].id) < x001067_g_StoneList[nLevel].num   then
		BeginEvent(sceneId)
	...
	local nItemBagIndexStone = GetBagPosByItemSn(sceneId, selfId, x001067_g_StoneList[nLevel].id)
	local szTransferStone = GetBagItemTransfer(sceneId,selfId, nItemBagIndexStone)
	
	-- 删除相关的石头
	local bDelOk = LuaFnDelAvailableItem(sceneId,selfId, x001067_g_StoneList[nLevel].id, x001067_g_StoneList[nLevel].num)
...
function x001067_TipBox( sceneId, selfId, msg )
	BeginEvent(sceneId)
	AddText(sceneId,msg);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
