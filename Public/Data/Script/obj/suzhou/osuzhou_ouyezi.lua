...
x001066_g_Name			= "欧冶子"
x001066_g_LeveUpScriptId = 500506 -- 神器升级脚本。
x001066_g_eventList={}
--**********************************
--事件列表
--**********************************
...
---------------------------介绍-------------------------------------------------
		...
		if GetNumText() == 105 then
			BeginEvent(sceneId)						
				...
				CallScriptFunction( x001066_g_LeveUpScriptId, "OnEnumerate",sceneId, selfId, targetId )
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
			return
		end	
		...
		if eventId == x001066_g_LeveUpScriptId then
			CallScriptFunction( eventId, "OnDefaultEvent", sceneId, selfId, targetId )
			return
		end	
