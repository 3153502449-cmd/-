...
x013001_g_eventList={228900,229009,200096,200097,808004,229012, 808092}
--**********************************
--事件交互入口
--**********************************
...
function x013001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			if x013001_g_MenPai == 6 then
				BeginEvent(sceneId)
					AddText(sceneId, "你又来消遣为师了，你已是我天龙弟子，还拜什么师呢。")
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,targetId)
			--返回值为9表示无门派
			elseif x013001_g_MenPai==9	then
				LuaFnJoinMenpai(sceneId, selfId, 1, 6)
		...
		LuaFnSetXinFaLevel(sceneId,selfId,42,60)				
				...
				CallScriptFunction( 888903, "PlayerJoinMenPai",sceneId, selfId, targetId )	 
				if	LuaFnGetSex( sceneId, selfId)==0	then
					LuaFnMsg2Player( sceneId, selfId,"你已经加入天龙派！",MSG2PLAYER_PARA)
	...
	if GetNumText()==6 then
		...
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 88, 44, "本观" )
		return
	end
end
