...
function x017505_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"#{XMPTM_130813_13}")
		AddNumText(sceneId, x017505_g_ScriptId, "学习唐门轻功",12,0)
	EndEvent(sceneId)
...
function x017505_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText()==0	then
		...
		if GetMenPai(sceneId, selfId) == 11 then
			if	HaveSkill( sceneId, selfId, 101)<0	then
				-- 检查钱
		...
		elseif GetMenPai(sceneId, selfId) == 9 then
			BeginEvent(sceneId)
				AddText(sceneId,"学习唐门轻功需要先加入唐门！")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		else
			BeginEvent(sceneId)
				AddText(sceneId,"你不是本门派的弟子，我是不能教你唐门轻功的")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end	
