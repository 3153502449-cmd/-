...
x712000_g_rare =
{
	...
	[7] = {Id = 38000011, Name=  "个4级宝石兑换券", Odds = 5000 },
	[8] = {Id = 20501003, Name=  "个3级棉布", Odds = 50000 },
    [9] = {Id = 20502003, Name=  "个3级秘银", Odds = 50000 },
}

-- name: 鱼的名字 mainId: 主要掉出物 needLevel: 钓此鱼需要的级别
x712000_g_GPInfo[201] = { name = "草鱼", mainId = 20102001, rareId = {7,8,9,}, needLevel = 1,}
...
function x712000_OnProcOver( sceneId, selfId, targetId )
	...
	for i=1, rareNum do
		...
		if(random(x712000_g_RandNum) <= rareInfo.Odds) then
			...
			if(ret1 > 0) then
				AddGlobalCountNews( sceneId,"#P少侠#W"..GetName(sceneId,selfId).."#P因仇家过多,躲在洛阳城内安逸钓鱼,却意外的钓到了#G一"..rareInfo.Name.."。")
