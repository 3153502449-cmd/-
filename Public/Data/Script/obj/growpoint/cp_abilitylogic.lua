...
local g_SplitGem_YuanBao = {
	...
	local gemQual = GetItemQuality( gem_type )	
	if gemQual <1 or gemQual >9 then  
	return 8 
	end
 local wupin = 0 
 for v=gemQual , 9 do 
	if LuaFnGetAvailableItemCount(sceneId, selfId, g_SplitGem_MatTab[v]) >0 then 
		wupin= g_SplitGem_MatTab[v]
		break
	end
  end	
if MatIndex == 70  then	
...
if LuaFnDelAvailableItem(sceneId,selfId,wupin, 1) >0 then
	return 9
end

elseif MatIndex == 71  then	
	local yuanbao =YuanBao(sceneId,selfId,-1,3,0)
	local qwupin =0
	if wupin ==0 then 
		qwupin = g_SplitGem_MatTab[gemQual]
	end
	if qwupin ==0 then return 8 end
	if 	yuanbao < g_SplitGem_YuanBao[gemQual] then 
	BeginEvent( sceneId )
		AddText( sceneId, "你的元宝不足。" )
	EndEvent( sceneId )
	DispatchMissionTips( sceneId, selfId )
		return 2
		else
...
function	x701601_SplitGemSucceed(sceneId,selfId, EquipPos, GemPos, MatPos)
	if MatPos ==71 then
		if YuanBao(sceneId,selfId,-1,2,g_SplitGem_YuanBao[gemQual]) ==1 then 
