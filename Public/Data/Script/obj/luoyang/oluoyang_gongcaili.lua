...
function x000132_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		...
		if 	PlayerName == "大师傅阿斯顿发大师傅打发第三方打发的斯蒂芬大师傅阿道夫阿道夫阿凡达" or PlayerName == "大师傅阿斯顿发大师傅打发第三方打发的斯蒂芬大师傅阿道夫阿道夫阿凡达" then
			----下面是每日充值
			...
			if lasttime == nowtime then
				--SetMissionData(sceneId,selfId,MD_EVERDATPAY,MyDatPrize+100)	--写入充值
				local Button = GetMissionData(sceneId,selfId,MD_EVERDATPAYBUTTN)
				local MyDatPrize = MyDatPrize+100
				local ButtonReq = {}
			...
			--SetMissionData(sceneId,selfId,MD_EVERDATPAYBUTTN,nButtnData)	--写入可领取	 
			end
