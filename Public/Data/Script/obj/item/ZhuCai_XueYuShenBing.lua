...
x335135_g_MaxCompleteValue = 95-- 最大神材完成度上限
...
--神材合成对应表
                         x335135_g_ShenCaiTable = {{sjid=30505700,sfid=30505800,lrid=30505900,cis=20},
													...
													{sjid=30505706,sfid=30505806,lrid=30505906,cis=5},}		-- -- 褚少微 2008.5.20。 102级神器铸造图样。残缺的神节7级，新莽神符7级，连戎神节7级。
...
function x335135_OnConditionCheck( sceneId, selfId )
	...
	--x335135_g_CompleteValue
	
	if Item01 == 30505800 then
	  x335135_g_CompleteValue = 5
	  x335135_g_MaxCompleteValue = 24
	elseif Item01 == 30505801 then
	x335135_g_MaxCompleteValue = 49
	x335135_g_CompleteValue = 5
	elseif Item01 == 30505802 then
	x335135_g_MaxCompleteValue = 74
	x335135_g_CompleteValue = 5
	end
	...
	local CompleteValue = GetBagItemParam( sceneId, selfId, sjbagpos, 8, 2 )
		...
		if CompleteValue >= x335135_g_MaxCompleteValue then		-- 当完整度达到100时
			...
			if ret > 0 then
				...
				local bagpos01 = TryRecieveItem( sceneId, selfId, x335135_g_ShenCaiTable[x335135_g_ShenCaiIndex].lrid, QUALITY_MUST_BE_CHANGE )	-- 放不下就没有了							
				local szItemTransfer = GetBagItemTransfer( sceneId, selfId, bagpos01 )
		              local strText = format("@*;SrvMsg;SCA:#G[系统]:#G轰隆隆,天空雷电交加，恭喜#{_INFOUSR%s}经过千辛万苦,取得终极神器打造材料#{_INFOMSG%s}! ", playername, szItemTransfer)									
			...
			LuaFnSendSpecificImpactToUnit( sceneId, selfId, selfId, selfId, x335135_g_Impact_Complete, 0 )	
              ...
              LuaFnDelAvailableItem(sceneId, selfId, Item01, 1)
	...
	CompleteValue = CompleteValue * 2
		...
		local strNotice = "你的".."#{_ITEM"..(x335135_g_ShenCaiTable[x335135_g_ShenCaiIndex].sjid).."}".."完整度增加了"..CompleteValue.."%,现在修复度为"..CompleteValue.."%,继续加油哦！" -- 残缺的神节
		x335135_ShowMsg( sceneId, selfId, strNotice)

	-- PrintStr("CheckParam ="..CheckParam)
	
	--if CheckParam < x335135_g_MaxCompleteValue and CheckParam + x335135_g_CompleteValue >= x335135_g_MaxCompleteValue then		-- 当完整度达到100时是否还有空间
		local FreeSpace = LuaFnGetPropertyBagSpace( sceneId, selfId )
		if( FreeSpace < 1 ) then
	BeginEvent( sceneId )
	AddText( sceneId, "您的物品栏没有空间，需要整理！" )
	EndEvent( sceneId )
	...
	if Item01 == 30505806 then
		local playername = GetName(sceneId, selfId)
	        --local nIndex = LuaFnGetBagIndexOfUsedItem( sceneId, selfId )	
		x335135_g_XinMang7JiInfo = GetItemName( sceneId, 30505806 )
		local strText = format("#{_INFOUSR%s}#{DQSJ_20080512_12}#cFF0000新莽神符7级#{DQSJ_20080512_13}", playername, GetItemName( sceneId, 30505806 ))						
		BroadMsgByChatPipe(sceneId,selfId, strText, 4)
...
function x335135_OnActivateOnce( sceneId, selfId )
	...
	--if nIndex >= 0 then		
		...
		local strNotice = "你的".."#{_ITEM"..(x335135_g_ShenCaiTable[x335135_g_ShenCaiIndex].sjid).."}".."完整度增加了5%" -- 残缺的神节
		x335135_ShowMsg( sceneId, selfId, strNotice)
