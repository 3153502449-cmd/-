...
function x005112_UpdateEventList( sceneId, selfId, targetId )
	...
	if nYaoDingCount <= 0 then
		BeginEvent(sceneId)
			...
			AddText(sceneId,"灵药已经炼制成功，只有炼药的人才能取得灵药——造化丹。")
	...
	elseif nYaoDingCount >= 1 then
		...
		BeginEvent(sceneId)
			...
			AddText(sceneId,"灵药——造化丹已经炼制成功，请问您是否现在要领取？")
			AddNumText(sceneId, x005112_g_ScriptId, "领取造化丹", 2, 0)
...
function x005112_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if	GetNumText()==0 then
		...
		if TryRecieveItem( sceneId, selfId, 38000641, 1 ) >= 0 then
			str		= "#Y你获得了"..GetItemName( sceneId, 38000641 ).."。"
			...
			--统计信息
			local guidSelf = 
			LuaFnAuditHDXianCaoZhengDuo(sceneId, selfId, "造化丹成");
